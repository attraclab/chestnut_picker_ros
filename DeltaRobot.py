...
class DeltaRobot:
	def __init__(self):
		...
		self.maxR = 650	#560.0 			#300 delta MK1				
		self.minStoke = -270.0		#-300 delta MK1	
		...
		self.GRIPPER_CLOSE = 2065	#1938	#1876
		self.GRIP_MID = 2277
		self.minObjectExisted = -30	#-95		# This is current value when it grab something
		...
		self.DEVICENAME                  = '/dev/u2d2'    # Check which port is being used on your controller
		...
		if ( (self.GetOperatingMode(self.DXL1_ID)==3) and (self.GetOperatingMode(self.DXL2_ID)==3) and (self.GetOperatingMode(self.DXL3_ID)==3) ):
			...
			# Set PID gain 
			P_Gain1 = 1000    #800 default
			I_Gain1 = 0     #0 default
			D_Gain1 = 4000   #4700 default

			P_Gain2 = 1000    #800 default
			I_Gain2 = 0     #0 default
			D_Gain2 = 4000   #4700 default

			P_Gain3 = 1000    #800 default
			I_Gain3 = 0     #0 default
			D_Gain3 = 4000   #4700 default

			P_Gain4 = 250    #800 default
			I_Gain4 = 0     #0 default
			D_Gain4 = 1400   #4700 default
	...
	def objectOnGripper(self):
		...
		if (self.ReadGripperPos() > self.GRIPPER_CLOSE) or (self.ReadGripperCurrent() < self.minObjectExisted):
			return True
		else:
			return False
				...
				if ((period > self.halfTime) and oneIsDone and twoIsDone and threeIsDone):
					...
					if grip:
						...
						if (self.objectOnGripper()):
							...
							## we ignore object checking, so True True
